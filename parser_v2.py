...
def extract_additional_safety_info(text):
    def extract_section(text, section_number, title = None):
        lines = text.splitlines()
        section_lines = []
        inside = False

        # can include "section 1. title" or "1. title"
        # for sigma aldrich and aaronchem, respectively
        next_section_pattern = rf"(section\s*)?{section_number + 1}[\.:]?"

        for line in lines:
            if title:
                if re.search(rf"(section\s*)?{section_number}[\.:]?\s*{title}", line, re.IGNORECASE):
                    inside = True
            else:
                if re.search(rf"(section\s*)?{section_number}[\.:]?", line, re.IGNORECASE):
                    inside = True

            if inside and re.search(next_section_pattern, line, re.IGNORECASE):
                break
            elif inside:
                section_lines.append(line.strip())
    ...
    section_7 = extract_section(text, 7, r"handling\s+and\s+storage")
    print("\n\nsection 7 text", section_7)
    section_9 = extract_section(text, 9, r"physical\s+and\s+chemical\s+properties")
    print("\n\nsection 9 text", section_9)
    section_10 = extract_section(text, 10, r"stability\s+and\s+reactivity")
